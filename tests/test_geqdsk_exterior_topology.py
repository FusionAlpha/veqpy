...
def test_vacuum_exterior_confines_closed_contours_to_lcfs_collar() -> None:
    ...
    R_surfaces = 6.0 + r[:, None] * (boundary_R[None, :] - 6.0)
    Z_surfaces = 0.4 + r[:, None] * (boundary_Z[None, :] - 0.4)
    psi_axis = -2.0
    normalized_grids = []
...
def test_explicit_psi_outside_retains_fixed_value_override() -> None:
    R = np.linspace(2.0, 10.0, 33)
    Z = np.linspace(-5.0, 6.0, 35)
    R_grid, Z_grid = np.meshgrid(R, Z, indexing="ij")
    theta = np.linspace(0.0, 2.0 * np.pi, 97, endpoint=False)
    boundary_R = 6.0 + 2.2 * np.cos(theta)
    boundary_Z = 0.4 + 3.5 * np.sin(theta)
    boundary = np.column_stack((boundary_R, boundary_Z))
    outside = ~Path(boundary, closed=True).contains_points(
        np.column_stack((R_grid.reshape(-1), Z_grid.reshape(-1)))
    ).reshape(R_grid.shape)
    ...
    psi_outside = 7.25
    psi_grid = _interpolate_psin_to_rectilinear_grid(
        6.0 + r[:, None] * (boundary_R[None, :] - 6.0),
        0.4 + r[:, None] * (boundary_Z[None, :] - 0.4),
        np.square(r),
        np.square(r),
        R_nodes=R,
        Z_nodes=Z,
    ...
    np.testing.assert_array_equal(psi_grid[outside], psi_outside)
